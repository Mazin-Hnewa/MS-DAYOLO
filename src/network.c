...
void forward_network(network net, network_state state)
{
    ...
    for(i = 0; i < net.n; ++i){
        ...
        */
        ...
        /*if (l.out_w >= 0 && l.out_h >= 1 && l.c >= 3 && i==0) {
                int j;
                for (j = 0; j < l.out_c; ++j) {
                    ...
                    char buff[256];
                    sprintf(buff, "feautres_0/layer-%d slice-%d", i, j);
                    //show_image(img, buff);
                    save_image(img, buff);
                }
...
float da_train_network_waitkey(network net, data d, data d_t, int wait_key)
{
    assert(d.X.rows % net.batch == 0);
    assert(d_t.X.rows % net.batch == 0);
    int batch = net.batch;
    int n = (d.X.rows + d_t.X.rows) / batch;
    float* X = (float*)xcalloc(batch * d.X.cols, sizeof(float));
    float* y = (float*)xcalloc(batch * d.y.cols/2, sizeof(float));
    ...
    float da_loss = 0, nan=0;
    // int lay=104;
    // layer see=net.layers[lay];
    // float see_sum=0,see_max=0, see_min=9999999;
    for(i = 0; i < n; ++i){
        ...
        sum += err;
        // #ifdef GPU
        // cuda_pull_array(see.delta_gpu, see.delta, see.outputs * see.batch);
        // #endif
        // //for (k=see.outputs*see.batch/2;k<see.outputs*see.batch ;++k)
        // for (k=0;k<see.outputs * see.batch;++k)
        // {see_sum+=see.delta[k];
        //   if (see.delta[k]>see_max) see_max=see.delta[k];
...
int resize_network(network *net, int w, int h)
{
    ...
    for (i = 0; i < net->n; ++i){
        ...
        if(l.type == CONVOLUTIONAL){
            ...
            resize_grl_layer(&l, w, h);
        }else if(l.type == DC){
            resize_dc_layer(&l, w, h);
