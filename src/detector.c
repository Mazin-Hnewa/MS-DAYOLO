...
void train_detector(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear, int dont_show, int calc_map, int mjpeg_port, int show_imgs, int benchmark_layers, int da, int epochs_map, char* chart_path)
{
    ...
    data train, buffer ,train_target , buffer_target;
    ...
    pthread_t load_thread_target;
    load_args args_target = { 0 };

    if (da)
    {
      printf("******************* domain adpatation ***************************\n");
    char *train_images_target = option_find_str(options, "train_target", "data/train_target.txt");
      list *plist_target = get_paths(train_images_target);
     int train_images_num_target = plist_target->size;
      char **paths_target = (char **)list_to_array(plist_target);
      net.train_images_num_target = train_images_num_target;

      args_target =args;
      args_target.d = &buffer_target;
      args_target.paths = paths_target;
      args_target.m = plist_target->size;
      args.n = imgs/2;
      args_target.n = imgs/2;

    }

    int calc_map_for_each = epochs_map * train_images_num / (net.batch * net.subdivisions);  // calculate mAP for each 4 Epochs
    if (!da) calc_map_for_each/=2;
    calc_map_for_each/=10;
    calc_map_for_each*=10;
    calc_map_for_each = fmax(calc_map_for_each, 100);
    printf("----------------  calc mAP for iterations:  %d -------------------------------\n",calc_map_for_each);
      ...
      printf("----------------  iteration %d -------------------------------\n",++count);
        if (l.random && count++ % 10 == 0) {
            ...
            free_data(train);
              ...
              if (da)
             {
              load_thread_target = load_data(args_target);
               pthread_join(load_thread_target, 0);
             train_target = buffer_target;
             free_data(train_target);}
        ...
        */
        ...
        if (ngpus == 1) {
            ...
            if (da)
            loss = da_train_network_waitkey(net, train, train_target, wait_key);
        ...
        if (net.cudnn_half) {
            if (iteration < net.burn_in * 3) fprintf(stderr, " Tensor Cores are disabled until the first %d iterations are reached.\n", 3 * net.burn_in);
        ...
        if (calc_map && (iteration >= next_map_calc || iteration == net.max_batches))
         {
            ...
            draw_precision = 1;
            if (net.max_iter>0)
            {for (int j = 0; j < net.no_grl; ++j)
            { printf(" weight of grl layer %d before change is %f \n",net.grl[j],net.layers[net.grl[j]].grl_weight);
              net.layers[net.grl[j]].grl_weight=(2/(1+exp(-10*(iteration/(float)net.max_iter))))-1;
              printf(" after change weight of grl layer %d to %f   \n",net.grl[j],net.layers[net.grl[j]].grl_weight);
            }
    ...
    { load_thread_target = load_data(args_target);
    pthread_join(load_thread_target, 0);
    free_data(buffer_target);
    free_load_threads(&args_target);
    //free(paths_target);
    //free_list_contents(plist_target);
    //free_list(plist_target);
  }
...
static void print_cocos(FILE *fp, char *image_path, detection *dets, int num_boxes, int classes, int w, int h, int image_id)
{
    int i, j;
...
void validate_detector(char *datacfg, char *cfgfile, char *weightfile, char *outfile)
{
    ...
    for (i = nthreads; i < m + nthreads; i += nthreads) {
        ...
        for (t = 0; t < nthreads && i + t - nthreads < m; ++t) {
            ...
            if (coco) {
                print_cocos(fp, path, dets, nboxes, classes, w, h,i+t-4);
            }
            else if (imagenet) {
    ...
    int letter_box, int benchmark_layers)
{
    ...
    while (1) {
        ...
        draw_detections_v3(sized, dets, nboxes, thresh, names, alphabet, l.classes, 1,"yolo");
...
void run_detector(int argc, char **argv)
{
    ...
    int da = find_arg(argc, argv, "-da");
    int epochs_map = find_float_arg(argc, argv, "-epochs_map", 4);
    ...
    else if (0 == strcmp(argv[2], "train")) train_detector(datacfg, cfg, weights, gpus, ngpus, clear, dont_show, calc_map, mjpeg_port, show_imgs, benchmark_layers , da, epochs_map, chart_path);
