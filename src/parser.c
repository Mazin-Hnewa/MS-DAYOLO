...
LAYER_TYPE string_to_layer_type(char * type)
{
    ...
    if (strcmp(type, "[grl]")==0) return GRL;
    if (strcmp(type, "[dc]")==0) return DC;
...
route_layer parse_route(list *options, size_params params)
{
    ...
    fprintf(stderr, " -> %4d x%4d x%4d   %d\n", layer.out_w, layer.out_h, layer.out_c, layer.batch);

    return layer;
}


layer parse_grl(list *options, size_params params)
{

   float grl_weight = option_find_float (options, "weight", 0.1);
   layer layer=make_grl_layer(params.batch, grl_weight, params.w, params.h, params.c);
   fprintf(stderr, "%4d x%4d x%4d  -> %4d x%4d x%4d    %d \n", layer.w, layer.h, layer.c, layer.out_w, layer.out_h, layer.out_c,layer.batch);
    return layer;
}


layer parse_dc(list *options, size_params params)
{
  if (params.c != 1) {
      printf("Error: filters= in the [previous convolutional]-layer must equal to 1 \n");
      exit(EXIT_FAILURE);
    }
    float dc_weight = option_find_float (options, "weight", 1);
    layer layer = make_dc_layer(params.batch, dc_weight, params.w, params.h);
     fprintf(stderr, "           %4d x%4d   -> %4d x%4d      %d\n", layer.w, layer.h , layer.out_w, layer.out_h,layer.batch);
    return layer;
}
...
void parse_net_options(list *options, network *net)
{
    ...
    net->max_iter = option_find_int(options, "max_iter", 0);
...
network parse_network_cfg_custom(char *filename, int batch, int time_steps)
{
    ...
    net.no_dc=0;
    net.dc = (int*)xcalloc(5, sizeof(float));
    net.no_grl=0;
    net.grl = (int*)xcalloc(5, sizeof(float));
    ...
    int count = 0;
    free_section(s);
    fprintf(stderr, "   layer   filters  size/strd(dil)      input                output             batch   \n");
    while(n){
        params.index = count;
        ...
        l.use_target = option_find_int_quiet(options, "use_target", 1);
        if (l.use_target==0 && params.batch!=1)  params.batch=net.batch/2;
        else  params.batch=net.batch;
        ...
        if(lt == CONVOLUTIONAL){
            ...
            l = parse_grl(options, params);
            net.grl[net.no_grl]=count;
            net.no_grl+=1;
        }else if(lt == DC){
            l = parse_dc(options, params);
            net.dc[net.no_dc]=count;
            net.no_dc+=1;
     ...
     int j;
    fprintf(stderr, "No. of DC layer = %d:  ", net.no_dc);
    for (j = 0; j < net.no_dc; ++j) fprintf(stderr, "%d, ", net.dc[j]);
    fprintf(stderr, " \n");
